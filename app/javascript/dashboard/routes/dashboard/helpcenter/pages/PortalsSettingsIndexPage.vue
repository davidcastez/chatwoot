...
const { isOnQualiChatCloud } = useAccount();
...
const fetchSSLStatus = () => {
  if (!isOnQualiChatCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on QualiChat cloud)
  if (portalObj?.custom_domain && isOnQualiChatCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
